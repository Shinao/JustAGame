...
namespace thor
{
  ...
  void UniversalEmitter::operator() (EmissionInterface& system, sf::Time dt, const sf::Vector2f &adjustment)
  {
    adjust(adjustment);
  ...
  void UniversalEmitter::adjust(const sf::Vector2f &adjustment)
  {
    _adjustment += adjustment;
  }

  void UniversalEmitter::emitParticle(EmissionInterface &system, int number)
  {
    // Create particle and specify parameters
    while (number--)
    {
      Particle particle( mParticleLifetime() );
      particle.position = mParticlePosition() + _adjustment;
