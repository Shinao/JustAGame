...
Connection ParticleSystem::addEmitter(std::function<void(EmissionInterface&, sf::Time, const sf::Vector2f&)> emitter)
{
	return addEmitter(emitter, sf::Time::Zero);
}

Connection ParticleSystem::addEmitter(std::function<void(EmissionInterface&, sf::Time, const sf::Vector2f&)> emitter, sf::Time timeUntilRemoval)
{
...
void ParticleSystem::update(sf::Time dt, const sf::Vector2f &camera_adjustment)
{
	...
	{
		itr->function(*this, dt, camera_adjustment);
	...
	for (ParticleContainer::iterator reader = mParticles.begin(); reader != mParticles.end(); ++reader)
	{
		...
		updateParticle(*reader, dt, camera_adjustment);
...
void ParticleSystem::updateParticle(Particle& particle, sf::Time dt, const sf::Vector2f &camera_adjustment)
{
	particle.passedLifetime += dt;

	particle.position += dt.asSeconds() * particle.velocity + camera_adjustment;
