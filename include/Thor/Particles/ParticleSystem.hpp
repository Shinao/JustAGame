...
class THOR_API ParticleSystem : public sf::Drawable, private sf::NonCopyable, public EmissionInterface
{
	// ---------------------------------------------------------------------------------------------------------------------------
	// Private types
	private:
		...
		typedef Function<void(EmissionInterface&, sf::Time, const sf::Vector2f&)>	Emitter;
	...
	public:
		...
		Connection					addEmitter(std::function<void(EmissionInterface&, sf::Time, const sf::Vector2f&)> emitter);
		...
		/// @return Object that can be used to disconnect (remove) the emitter from the system.
		Connection					addEmitter(std::function<void(EmissionInterface&, sf::Time, const sf::Vector2f&)> emitter, sf::Time timeUntilRemoval);
		...
		void						update(sf::Time dt, const sf::Vector2f &camera_adjustment = sf::Vector2f(0, 0));
	...
	private:
		...
		void						updateParticle(Particle& particle, sf::Time dt, const sf::Vector2f &camera_adjustment);
