...
std::function<void(EmissionInterface&, sf::Time, const sf::Vector2f&)> refEmitter(Emitter& referenced)
{
	return [&referenced] (EmissionInterface& system, sf::Time dt, const sf::Vector2f &adjustment)
	{
		return referenced(system, dt, adjustment);
	};
}
...
class THOR_API UniversalEmitter
{
	...
	public:
		...
		void						operator() (EmissionInterface& system, sf::Time dt, const sf::Vector2f &adjustment);
		...
		void emitParticle(EmissionInterface &system, int number = 1);
		void adjust(const sf::Vector2f &adjustment);
